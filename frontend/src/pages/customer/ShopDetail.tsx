...
export function ShopDetail() {
  ...
  const handleAddToCart = (product: any) => {
    if (!data) return;
    const shopName = getLocalized(data.name, i18n.language);
    const productName = getLocalized(product.name, i18n.language);
    addItem(data._id, shopName, {
  ...
  const filteredProducts = data?.products?.filter((product: any) => {
    const name = getLocalized(product.name, i18n.language).toLowerCase();
    const desc = getLocalized(product.description, i18n.language).toLowerCase();
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">{t('shop_detail.not_found')}</h2>
  ...
  const shopName = getLocalized(data.name, i18n.language);
  const shopDesc = getLocalized(data.description, i18n.language);
    ...
    <div className="min-h-screen bg-gray-950">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-start">
            <div className="w-20 h-20 rounded-2xl overflow-hidden bg-white/10 border border-white/10 flex-shrink-0">
              {data.logoUrl ? (
                <img src={getImageUrl(data.logoUrl)} alt={shopName} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="flex-1">
              ...
              <p className="text-white/50 text-sm mb-4 max-w-xl">{shopDesc}</p>
              <div className="flex flex-wrap gap-4 text-sm text-white/40">
                <div className="flex items-center gap-1.5">
                  <MapPin className="w-4 h-4" />
                  {getLocalized(data.address, i18n.language)}
                </div>
                <div className="flex items-center gap-1.5">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="relative max-w-md mb-8">
          ...
          <input
            type="text"
            placeholder={t('shop_detail.search')}
            value={searchQuery}
        ...
        {filteredProducts && filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-24">
            ...
            <h3 className="text-xl font-bold text-white mb-2">{t('shop_detail.no_products')}</h3>
            <p className="text-white/40 text-sm">{t('shop_detail.no_products_desc')}</p>
          </div>
        )}
      </div>
    </div>
