    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <section className="relative py-12 px-4 bg-gradient-to-br from-gray-900 via-primary-950 to-primary-900 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <p className="text-white/50 mt-2">{items.length} {t('cart.items')} · {formatPrice(totalAmount)}</p>
        </div>
      </section>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="text-base font-bold text-white">{t('checkout.delivery_address')}</h2>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea placeholder={t('checkout.notes_placeholder')} rows={3}
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="text-base font-bold text-white">{t('checkout.payment')}</h2>
              </div>

              <div className="flex items-center gap-3 p-4 rounded-xl border-2 border-primary-500/60 bg-primary-500/10">
                <Wallet className="w-6 h-6 text-primary-400" />
                <div>
                  <p className="font-semibold text-sm text-white">{t('checkout.cash')}</p>
                  <p className="text-xs text-white/30 mt-0.5">{t('checkout.cash_desc')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white/5 border border-white/10 rounded-2xl p-6">
              <h2 className="text-base font-bold text-white mb-5">{t('cart.order_summary')}</h2>
              ...
              <div className="border-t border-white/10 pt-4 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-white/60 text-sm font-medium">{t('cart.total')}</span>
              ...
              >
                {isLoading ? t('checkout.placing') : (
                  <>{t('checkout.place_order')} <ArrowRight className="w-4 h-4" /></>
                )}
              </button>
